        ...
        <Page size="A4" style={{ color: "#000", fontFamily: "DM-Sans", fontWeight: "normal", paddingTop: 56, paddingHorizontal: 64, position: 'relative' }}>
            ...
            <View style={styles.viewQuestionMoreMargin}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <Text style={styles.answer}>[ {typeof briefingData?.resources != 'string' && briefingData?.resources!?.filter(el => el == ResourcesEnum.carouselBanner).length > 0 ? "x" : ""} ]&nbsp;&nbsp;{ResourcesEnum.carouselBanner}</Text>
                    <Text style={styles.answer}>[ {typeof briefingData?.resources != 'string' && briefingData?.resources!?.filter(el => el == ResourcesEnum.contentManager).length > 0 ? "x" : ""} ]&nbsp;&nbsp;{ResourcesEnum.contentManager}</Text>
                </View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <Text style={styles.answer}>[ {typeof briefingData?.resources != 'string' && briefingData?.resources!?.filter(el => el == ResourcesEnum.googleMaps).length > 0 ? "x" : ""} ]&nbsp;&nbsp;{ResourcesEnum.googleMaps}</Text>
                    <Text style={styles.answer}>[ {typeof briefingData?.resources != 'string' && briefingData?.resources!?.filter(el => el == ResourcesEnum.instagram).length > 0 ? "x" : ""} ]&nbsp;&nbsp;{ResourcesEnum.instagram}</Text>
                </View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <Text style={styles.answer}>[ {typeof briefingData?.resources != 'string' && briefingData?.resources!?.filter(el => el == ResourcesEnum.multiLanguage).length > 0 ? "x" : ""} ]&nbsp;&nbsp;{ResourcesEnum.multiLanguage}</Text>
                    <Text style={styles.answer}>[ {typeof briefingData?.resources != 'string' && briefingData?.resources!?.filter(el => el == ResourcesEnum.newsletter).length > 0 ? "x" : ""} ]&nbsp;&nbsp;{ResourcesEnum.newsletter}</Text>
                </View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <Text style={styles.answer}>[ {typeof briefingData?.resources != 'string' && briefingData?.resources!?.filter(el => el == ResourcesEnum.onlineChat).length > 0 ? "x" : ""} ]&nbsp;&nbsp;{ResourcesEnum.onlineChat}</Text>
                    <Text style={styles.answer}>[ {typeof briefingData?.resources != 'string' && briefingData?.resources!?.filter(el => el == ResourcesEnum.ticketSystem).length > 0 ? "x" : ""} ]&nbsp;&nbsp;{ResourcesEnum.ticketSystem}</Text>
                </View>
            </View>
